      ...
      <template v-if="question.info.quesChoiceType===1">
        <ul class="listIww">
            <li 
              ...
              style="display:inline-flex;width: 100%; margin: 3px 0; "
            >
              <em>{{item.flag}}.</em>
...
export default {
  ...
  computed: {
    isReadOnly() {
      if (this.isComplete) {
        //查看上一题
        this.filter = this.$store.state.userSelectAnswerArr[this.nowIndex] * 1;
  ...
  methods: {
    ...
    submitQues() {
      ...
      if (typeof this.question.info.quesChoice[this.filter] == "undefined") {
        var userselect = "";
      } else {
        var userselect = this.filter;
      }

      this.$emit("addAnswerLog", this.question.id, this.isRight, userselect);
    ...
    showJiexiContent() {
      ...
      setTimeout(function() {
        ...
        // var scrollHeight = document.getElementById('jiexie').offsetTop;
        var scrollHeight = document.body.scrollHeight;
        window.scroll(0, scrollHeight);
      }, 200);
    },
