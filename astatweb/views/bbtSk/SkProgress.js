...
$(function() {// 初始化内容
	var url = furl + warName + "/getSKbbtList?school_id="+school_id+"&starttime="+starttime+"&endtime="+endtime;
	getUbName();
	setTable(url);
	setTime();
	setSelect("");
...
function setTable(url){
  ...
  table.render({
    ...
    ,cols: [[
	  ...
	  ,{field:'tls_get_ub', title: '授课获取'+ub_name+'数', sort: true}
      ,{field:'tls_login_times', title: '登录次数', sort: true}
      ,{field:'tls_res_times', title: '资源播放', sort: true}
      ,{field:'tls_praise_times', title: '表扬次数',sort: true}
	  ,{field:'tls_train_times', title: '训练次数',sort: true}
	  ,{field:'popular_value', title: '课堂报告人气值',sort: true}
    ]]
	,done:function(res){
        tdTitle();
    }
...
function search(){
	var url=getSearchUrl("getSKbbtList");
	setTable(url);
}
   //导出
function excl_rep(){

	var url=getSearchUrl("getSKbbtListExport");
	...
	$.ajax({
		...
		success: function(data){
			...
			layui.use(['excel'], function() {
				...
				var dataA = [
				  ['用户姓名','授课获取'+ub_name+'数','登录次数','资源播放','表扬次数','训练次数','课堂报告人气值']
				];
				if(data.data.length > 0){
					for(var i=0;i<data.data.length;i++){
						dataA.push([
						  data.data[i].truename,
						  data.data[i].tls_get_ub.toString(),
						  data.data[i].tls_login_times.toString(),
						  data.data[i].res_play_times.toString(),
						  data.data[i].tls_praise_times.toString(),
						  data.data[i].tls_train_times.toString(),
						  data.data[i].popular_value.toString()
						]);
					}
				}
				excel.exportExcel({
...
function getSearchUrl(method){
	...
	var skqk = $("#skqkList option:selected").val();
	var url=furl + warName+"/"+ method +"?school_id="+school_id+"&starttime=" + starttime + "&endtime=" + endtime + "&gtId=" + gtId + "&search=" + search + "&orderBy=" + orderBy + "&skqk=" + skqk;
	return url;
}
/** 
 * 获取指定的URL参数值 
...
function getParam(paramName) { 
	...
	function echartBK() {
		...
		var option = {
			...
			$.ajax({
				url: furl + warName + "/getSKbbtDayList",
				type: "post",
				...
				$("#SKbbtCount").text(res[1].SKbbtCount);
		...
		$.ajax({
				url: furl + warName + "/getSKbbtRate",
				type: "post",
				data: {"school_id":school_id,"starttime":starttime,"endtime":endtime},
				datatype: "json",
				success: function (res) {
					var tmp1 ="{value:"+res[0].SKbbtCount+",name:'使用班班通授课的教师'}";
					var tmp2 ="{value:"+res[1].unSKbbtCount+",name:'未使用班班通授课的教师'}";
