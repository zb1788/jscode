...
import { addColumn, getRuleDictionaryByType, getColRuleByColId } from '@/api/canel';
...
export default {
  ...
  data () {
    return {
      ...
      form: {
        ...
        colLength: this.$route.query.colLength || '',
        columnRuleList: [
          {}
        ]
      },
  ...
  async mounted () {
    let res = await getColRuleByColId(this.form.id);
    ...
    res.data.map((item, i) => {
      ...
      this.form.columnRuleList.push(obj);
    });
    if (this.form.columnRuleList.length === 0) {
      this.form.columnRuleList = [{}];
    }
  },
  watch: {
