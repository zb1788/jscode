  ...
  <div class="order_list">
        <van-list v-model="loading"
          ...
          <van-panel v-for="(el, i) in orderList"
            ...
            <div class="total" v-if="el.zhifustate == 2">兑换码：{{el.activity_code}}</div>
            ...
            <div slot="footer"
              ...
              <van-button size="small"
                          v-if="el.zhifustate == 1"
                          type="danger"
                          @click.stop="toPay(el.id,el.chanpinid)">去支付</van-button>
              <!-- <van-button size="small"
  ...
  methods: {
    ...
    async getOrderList() {
      ...
      if(res.code == 1){
        ...
        this.loading = false;        
        if (this.orderList.length>=this.total||this.total == 0) {
            this.finished = true;
        }
      }else{
    ...
    commentOrder(id) {},
    toPay(id,chanpinid) {
      this.$router.push('/msg/checkorder/'+id+'/'+chanpinid)
