<template>
  <div class="tab_home">
    ...
    <!-- <img style="height:230px" v-for="(img, index) in banner" :key="index" v-lazy="img" > -->
    <van-image
       style="height:230px" 
      width="100%"
      :src="lunbo"
...
<!-- 
    ...
    <!-- <van-image
      ...
      :key="index"
      :src="grouponGood.picUrl"
      @click="goBrand(grouponGood.id)"
    /> -->
      <van-card :thumb-link="goBrand(grouponGood.id)"
                ...
                @native-click="goBrand(grouponGood.id)">
    ...
    <!-- <van-panel title="全部服务">
      <van-grid clickable
                :column-num="2">
        <van-grid-item v-for="(brand ,index) in brandList"
                       :key="index"
                       :text="brand.name"
                       :url="goBrand(brand.id)">
          <img :src="brand.picUrl"
               style="width: 80%;" />
          <div style="width: 80%;font-size:16px;color: rgb(171, 149, 109);"> {{ brand.name }}  </div>
          <div style="width: 80%;font-size:16px;color: #f44;font-weight: 500;">
            <van-tag plain
                   type="primary">
            {{brand.cishu}}次
            </van-tag>
         ...
         <van-cell-group>
          <van-cell title="全部服务"
            ...
            <router-link to="/items/brand-list"
                         class="text-desc">更多服务</router-link>
          </van-cell>
        </van-cell-group>
      </div> 
    </van-panel> -->
    ...
    async initViews() {
      ...
      if(res.code == 1){
        for(let [index,item] of res.data.entries()){
          ...
          console.log(index)
          // if(index == 0){
          //   obj.picUrl = require('../../assets/images/home_lvsejiuzhen.jpg');
          // }else if(index == 1){
          //   obj.picUrl = require('../../assets/images/home_mingyiwenzhen.jpg');
          // }else if(index == 2){
          //   obj.picUrl = require('../../assets/images/home_mingyizhudao.jpg');
          // }
          obj.picUrl = item.pic;
