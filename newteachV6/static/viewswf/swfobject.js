...
var swfobject = function() {
	var D = "undefined", r = "object", T = "Shockwave Flash", Z = "ShockwaveFlash.ShockwaveFlash", q = "application/x-shockwave-flash", S = "SWFObjectExprInst", x = "onreadystatechange", Q = window, h = document, t = navigator, V = false, X = [], o = [], P = [], K = [], I, p, E, B, L = false, a = false, m, G, j = true, l = false, O = function() {
		var ad = typeof h.getElementById != D
				&& typeof h.getElementsByTagName != D
				&& typeof h.createElement != D, ak = t.userAgent.toLowerCase(), ab = t.platform
				.toLowerCase(), ah = ab ? /win/.test(ab) : /win/.test(ak), af = ab ? /mac/
				.test(ab)
				: /mac/.test(ak), ai = /webkit/.test(ak) ? parseFloat(ak
				.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : false, aa = t.appName === "Microsoft Internet Explorer", aj = [
				0, 0, 0 ], ae = null;
		if (typeof t.plugins != D && typeof t.plugins[T] == r) {
			...
			if (ae
				...
				aj[2] = /[a-zA-Z]/.test(ae) ? n(ae.replace(/^.*[a-zA-Z]+(.*)$/,
						"$1")) : 0
			}
		} else {
		...
		if ((typeof h.readyState != D && (h.readyState === "complete" || h.readyState === "interactive"))
	...
	function R(ad, ae, aa, ac) {
		...
		if (ah) {
			...
			ad.id = S;
			if (typeof ad.width == D
					|| (!/%$/.test(ad.width) && n(ad.width) < 310)) {
				ad.width = "310"
			}
			if (typeof ad.height == D
					|| (!/%$/.test(ad.height) && n(ad.height) < 137)) {
				ad.height = "137"
			}
			var ag = O.ie ? "ActiveX" : "PlugIn", af = "MMredirectURL="
					+ encodeURIComponent(Q.location.toString().replace(/&/g,
							"%26"))
					+ "&MMplayerType="
					+ ag
					+ "&MMdoctitle="
					+ encodeURIComponent(h.title.slice(0, 47)
	...
	function k(aa, ab) {
		var ac = C("div");
		ac.innerHTML = "<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000'><param name='movie' value='"
				+ aa + "'>" + ab + "</object>";
	...
	function N(ab) {
		...
		return aa && typeof encodeURIComponent != D ? encodeURIComponent(ab)
				: ab
	}
	var d = function() {
		if (O.ie) {
					...
					if (ad && typeof ad === r) {
						for ( var am in ad) {
							if (ad.hasOwnProperty(am)) {
								var ap = (l) ? encodeURIComponent(am) : am, at = (l) ? encodeURIComponent(ad[am])
										: ad[am];
	 ...
	 function showFlexPager(url)
		 ...
		 //alert(url)
		  var flashvars = { 
            ...
            var attributes = {};
           // attributes.id = "FlexPagerView";
           // attributes.name = "FlexPagerView";
					 // attributes.align = "middle";
					 var el = document.getElementById("flashContent");
					 var winWidth = document.getElementById("playDiv").clientWidth;
					 var winHeight = document.getElementById("playDiv").clientHeight;
					 debugger
					 swfobject.embedSWF(url, el, winWidth, winHeight*0.95, "9.0.0", "expressInstall.swf");
						
				// 	 swfobject.embedSWF(
				// 			url, el, 
				// 			document.body.clientWidth, document.body.clientHeight, 
                // flashvm.swfVersionStr, flashvm.xiSwfUrlStr, 
                // flashvars, params, attributes);
						//swfobject.createCSS(el, "display:inline-block;text-align:left;");
						
						
            
		  }
		  export function showFlexPager2(url,slef)
		  {
				//url="http://rss1zz.zzedu.net.cn:9000/material/20190315/20190315104746502656746247014/%E6%8F%90%E9%AB%98%E8%AE%AD%E7%BB%83%EF%BC%9A%E4%B8%A4%E4%BD%8D%E6%95%B0%E5%87%8F%E4%B8%80%E4%BD%8D%E6%95%B0%EF%BC%88%E9%80%80%E4%BD%8D%EF%BC%89.swf?rcode=c15f153d4c9a4dd4c959ff099410d45c7b20adc97e4c20c1c42493002ee4ed7f&validate_code=223afb7b42a64b5880bae8490af23032&username=00226&flashtype=%E8%94%BA%E9%87%91%E6%A2%85&ad=0022620190516160537_0&st=vod&ct=0&CLIENTIP=218.28.20.138"
				flashvm=self;
		  //url=encodeURIComponent(url);
		 // url=encodeURIComponent(url);
		  showFlexPager(url);
		  }
		  function showCommonFash(url)
		  {
		 // method2.movie=url;
		  //alert(url);
		    var params = {};
            ...
            params.allowfullscreen = "true";
            params.WMode = "transparent";
		   swfobject.embedSWF(url, "flashContent", "100%", "100%", swfVersionStr,xiSwfUrlStr,'',params);
		   swfobject.createCSS("#flashContent", "display:block;text-align:left;");
		  }
		//     function showFlexPagerSize(url,width,height)
		//   {
		//   url=encodeURIComponent(url);
		//  // url=encodeURIComponent(url);
		//   showFlexPagerSize(url,mainflashpath,width,height);
		//   }
		  function showFlexPagerSize(url,mainflashpath,width,height)
		  {
		 //alert(url)
		  var flashvars = { 
                      SwfFile :url,
		  ...
		  function showCommonFashSize(url,width,height)
		  {
		 // method2.movie=url;
		  //alert(url);
		    var params = {};
            ...
            params.allowfullscreen = "true";
            params.WMode = "transparent";
		   swfobject.embedSWF(url, "flashContent", width, height, swfVersionStr,xiSwfUrlStr,'',params);
		   swfobject.createCSS("#flashContent", "display:block;text-align:center;");
		  }
