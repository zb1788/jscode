...
<div>
  <div class="timu" style="display: block;">
    <p class="fenx" v-if="questionNew.info.quesName!=''" style="text-align:left;">{{questionNew.info.quesName}}</p>
    <!-- 普通算式 -->
    ...
    <div v-if="questionNew.info.quesContentType==2" class="fenx">
      ...
      >
        <template v-if="typeof item=='string'">
          ...
          <span v-if="item=='#'" :class="['fkong',findex==index1&&lindex==0?'item':'']" :key="index1" @click="choose(index1,0)">{{answerInput[index1+'-'+0]}}</span>
        </template>
        <template v-if="item.length>1 && typeof item !='string'" v-for="(ii,index2) in item">
...
export default {
  ...
  computed: {
    questionNew() {
      this.answerArr = [];
      this.filter = 0;
      console.log(this.question.info.quesContent);
