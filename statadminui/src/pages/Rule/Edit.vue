...
export default {
  ...
  data () {
    return {
      ...
      form: {
        ...
        ruleName: this.$route.query.ruleName || '',
        groupCode: this.$route.query.groupCode || '',
        taskType: this.$route.query.taskType || '',
        taskContent: this.$route.query.taskContent || '',
        maxTimes: this.$route.query.maxTimes || '',
        beforeRuleCode: this.$route.query.beforeRuleCode || '',
        remark1: this.$route.query.remark1 || ''
      },
      formItems: [
  ...
  methods: {
    ...
    async getRuleInfoListByGroupCode (groupCode) {
      ...
      let ruleList = [];
      ruleList.push({'label': '无前置条件', 'value': ''});
      res.data.length > 0 && res.data.map(item => {
        if (item.ruleCode !== this.form.ruleCode) {
          let rule = {};
