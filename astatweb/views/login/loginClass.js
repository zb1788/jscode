...
$(function() {// 初始化内容
	var url = furl + warName + "/getClazzLoginList?school_id="+school_id+"&starttime="+starttime+"&endtime="+endtime;
	setTable(url);
	setTime();
	echartBK();
});
function setTable(url){
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'active_persons', title: '激活人数', sort: true}
      ,{field:'login_persons', title: '登录人数', sort: true}
	  ,{field:'rate', title: '登录人数占比', sort: true,templet:function(d){
		  return d.rate+'%';
	  }}
    ]]
	,done:function(res){
	...
	table.on('sort(test-table-thead1)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
	  ...
	  table.reload('tableList', {
		...
		,url: getSearchUrl("getClazzLoginList")
...
function search(){
	var url=getSearchUrl("getClazzLoginList");
	setTable(url);
}
   //导出
function excl_rep(){

	var url=getSearchUrl("getClazzLoginListExport");
				...
				if(data.data.length > 0){
					for(var i=0;i<data.data.length;i++){
						dataA.push([
						  ...
						  data.data[i].active_persons.toString(),
						  data.data[i].login_persons.toString(),
						  data.data[i].rate.toString()+"%"
						]);
					}
				}
				excel.exportExcel({
