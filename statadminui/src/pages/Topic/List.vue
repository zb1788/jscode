...
import { getListByPage, delTopicById } from '@/api/canel';
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { prop: 'platid', label: '平台标识', minWidth: '150px' },
        { prop: 'topic', label: 'Topic名称', minWidth: '150px' },
        { prop: 'topicPartition', label: 'Topic分区号', minWidth: '120px' },
        { prop: 'topicOffset', label: '分区对应offset', minWidth: '140px' },
        { prop: '__edit',
          label: '操作',
          fixed: 'right',
          minWidth: '180px',
  ...
  methods: {
    ...
    async getStuMuti (index) {
      ...
      const res = await getListByPage({ ...this.pager, ...this.form });
      console.log(res);
      if (res.code === '200') {
        ...
        renderFooter: (h, $dialog) => {
          return <div>
            <vcom-button
              ...
              onClick={async () => {
                let id = row.id;
                const res = await delTopicById(id);
                if (res.code === '200') {
