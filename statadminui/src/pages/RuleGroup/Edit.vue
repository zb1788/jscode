...
import { addRuleGroup } from '@/api/rule';
export default {
  mixins: [form],
  components: {
    VcomForm
  ...
  data () {
    return {
      ...
      form: {
        ...
        groupRule: this.$route.query.groupRule || '',
        remark1: this.$route.query.remark1 || ''
      },
      formItems: [
        { label: '分组名称', field: 'groupName', required: true },
        { label: 'Cron表达式', field: 'groupRule', required: true },
  ...
  methods: {
    async handleAdd () {
      if (this.$refs.form.validator()) {
        ...
        let res = await addRuleGroup(this.form);
        console.log(res);
        if (res.code === '200') {
