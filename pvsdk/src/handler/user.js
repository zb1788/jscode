...
class User {
    ...
    get data () {
        ...
        var userinfo = this.info.info;
        var storage = window.localStorage;
        var tmpPlatform = storage.getItem("__VCOM_ANALYTICS_PLATFORM");
        if (undefined == tmpPlatform || tmpPlatform.length == 0) {
            tmpPlatform = config["default"].PLATFORM;
        }
        // vcom xuexiaofei add PV访问日志中增加访问平台标识 20190524 end
        return {
            // vcom xuexiaofei add PV访问日志中增加访问平台标识 20190524 start
            [columns.platform]: tmpPlatform,
            [columns.schoolid]: userinfo['schoolid'],
            // vcom xuexiaofei add PV访问日志中增加访问平台标识 20190524 end
    ...
    login (info) {
        ...
        if (typeof info === Object){
            if(info.schoolid == null||typeof info.schoolid == 'undefined'){
                console.warn('schoolid不能为空！');
                info.schoolid = undefined;
            }
            if(info.platform == null||typeof info.platform == 'undefined'){
                console.error('平台标识(platform)不能为空！');
                info.platform = undefined;
            }
            if(info.utm_uuid ==null||typeof info.utm_uuid == 'undefined'){
                info.platform = undefined;
                console.error('用户帐号(utm_uuid)不能为空！');
            }
        }
        // 判断是否曾经登录过
