...
$(function() {// 初始化内容
	var url = furl + warName + "/getTeacherLoginList?school_id="+school_id+"&starttime="+starttime+"&endtime="+endtime;
	setTable(url);
	setTime();
	setSelect("");
	echartBK();
...
function setTable(url){
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'login_times', title: '登录总次数'}
      ,{field:'online_time', title: '在线总时长（分钟）', sort: true}
	  ,{field:'por_login_times', title: '空间登录次数'}
	  ,{field:'tls_login_times', title: '授课端登录次数', sort: true}
	  ,{field:'tlspor_login_times', title: '空间+授课端登录次数', sort: true}
	...
	table.on('sort(test-table-thead1)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
	  ...
	  table.reload('tableList', {
		...
		,url: getSearchUrl("getTeacherLoginList")
...
function search(){
	var url=getSearchUrl("getTeacherLoginList");
	setTable(url);
}
   //导出
function excl_rep(){

	var url=getSearchUrl("getTeacherLoginListExport");
	...
	$.ajax({
		...
		success: function(data){
			...
			layui.use(['excel'], function() {
				...
				var dataA = [
				  ['用户姓名','机构/教研组','登录总次数','在线总时长（分钟）','空间登录次数','授课端登录次数','空间+授课端登录次数','手机APP登录次数',]
				];
				if(data.data.length > 0){
					for(var i=0;i<data.data.length;i++){
						dataA.push([
						  ...
						  data.data[i].tlspor_login_times.toString(),
...
function getParam(paramName) { 
	...
	function echartBK() {
		...
		var option = {
			...
			$.ajax({
				url: furl + warName + "/getTeacherLoginDayList",
				type: "post",
				...
				$("#TeacherLoginCount").text(res[1].TeacherLoginCount);
		...
		$.ajax({
				url: furl + warName + "/getTeacherLoginRate",
				type: "post",
				data: {"school_id":school_id,"starttime":starttime,"endtime":endtime},
				datatype: "json",
				success: function (res) {
					var tmp1 ="{value:"+res[0].TeacherLoginCount+",name:'登录教师数'}";
					var tmp2 ="{value:"+res[1].unTeacherLoginCount+",name:'未登录教师数'}";
