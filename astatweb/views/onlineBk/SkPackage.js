...
$(function() {// 初始化内容
	var url = furl + warName + "/getSKList?school_id="+school_id+"&starttime="+starttime+"&endtime="+endtime;
	setTable(url);
	setTime();
	setSelect("grade");
	setSelect("subject")
...
function setTable(url){
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'coursePackage', title: '课程及课时'}
	  ,{field:'truename', title: '共享者姓名'}
      ,{field:'resnum', title: '授课包资源个数'}
      ,{field:'share_number', title: '被引用次数', sort: true}
      ,{field:'last_share_time', title: '共享时间',sort: true}
    ]]
	,done:function(res){
        tdTitle();
    }
...
function setSelect(code){
	$.ajax({
		url:furl + warName+"/getListByCode?code="+code,
		type:"GET",
		dataType:"json",
		success:function(result){
			var list = result;    //返回的数据
			var dataList = document.getElementById(code+"List"); //server为select定义的id
			for(var p in list){
				var option = document.createElement("option");  // 创建添加option属性
				option.setAttribute("value",list[p].code); // 给option的value添加值
				option.innerText=list[p].code_name;     // 打印option对应的纯文本 
				dataList.appendChild(option);           //给select添加option子标签
...
function search(){
	var url=getSearchUrl("getSKList");
	setTable(url);
}
   //导出
function excl_rep(){

	var url=getSearchUrl("getSKListExport");
	...
	$.ajax({
		...
		success: function(data){
			...
			layui.use(['excel'], function() {
				...
				var dataA = [
				  ['课程及课时','共享者姓名','授课包资源个数','被引用次数','共享时间']
				];
				if(data.data.length > 0){
					for(var i=0;i<data.data.length;i++){
						dataA.push([
						  data.data[i].coursePackage,
						  data.data[i].truename,
						  data.data[i].resnum.toString(),
						  data.data[i].share_number.toString(),
						  data.data[i].last_share_time
						]);
					}
				}
				excel.exportExcel({
...
function getSearchUrl(method){
	...
	var grade = $("#gradeList option:selected").val();
	var subject = $("#subjectList option:selected").val();
	var gx = $("#gxList option:selected").val();
	var url=furl + warName+"/"+ method +"?school_id="+school_id+"&starttime=" + starttime + "&endtime=" + endtime + "&grade=" + grade + "&subject=" + subject +"&search=" + search + "&orderBy=" + orderBy + "&gx=" + gx;
	return url;
}
/** 
 * 获取指定的URL参数值 
...
function getParam(paramName) { 
	...
	function echartBK() {
		...
		/** 折线图（echarts设置） start */	
		myChart = echarts.init(document.getElementById("skPackageDay"));
		var dataX=[];
		var dataY=[];
		var option = {
				tooltip : {
		...
		/** 饼图（echarts设置） start */	
		myCharts1 = echarts.init(document.getElementById('skPackageDay1'));
		var dataA=[];
		var option1 = {
		  ...
		  legend: {
            ...
            data: ['使用授课包共享功能的教师','未使用授课包共享功能的教师']
          },
		  color:['#76acff','#ff827c','#f8ce3d','#28d752'],
		  toolbox: {
			show : false,
		...
		$.ajax({
				url: furl + warName + "/getSKRate",
				type: "post",
				data: {"school_id":school_id,"starttime":starttime,"endtime":endtime},
				datatype: "json",
				success: function (res) {
					var tmp1 ="{value:"+res[0].SKCount+",name:'使用授课包共享功能的教师'}";
					var tmp2 ="{value:"+res[1].unSKCount+",name:'未使用授课包共享功能的教师'}";
