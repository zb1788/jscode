...
$(function() {// 初始化内容
	setMsg();
	var url = furl + warName + "/getKtbgList?school_id="+school_id+"&starttime="+starttime+"&endtime="+endtime;
	setTable(url);
	setTime();
	setSelect("");
	echartBK();
...
function getHttpOrHttps(){
	var url = furl + warName+ "/getHttpOrHttps";
		$.ajax({
		type: "GET",
		url: url,
		dataType: "text",
...
function getIlearn(){
	var url = furl + warName+ "/getIlearn";
		$.ajax({
		type: "GET",
		url: url,
		dataType: "text",
...
function setMsg(){
	var url = furl + warName+ "/getPbktIsOrNot?school_id="+school_id;
	$.ajax({
	type: "GET",
	url: url,
	dataType: "text",
...
function setTable(url){
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'create_time', title: '上课时间', sort: true}
	  ,{field:'teacher_name', title: '教师姓名', sort: true}
      ,{field:'class_name', title: '上课班级'}
      ,{field:'report_name', title: '课堂报告',templet: function(d){
        return '<div><a href="'+httpOrHttps+'://'+ilearn+'/classReport/action/pc/view/classReport/'+d.report_id+'" class="layui-table-link" target="view_window">'+ d.report_name +'</a></div>';
      }}
    ]]
	,done:function(res){
        tdTitle();
...
function search(){
	var url=getSearchUrl("getKtbgList");
	setTable(url);
}
   //导出
function excl_rep(){

	var url=getSearchUrl("getKtbgListExport");
...
function getParam(paramName) { 
	...
	function echartBK() {
		...
		var option = {
			...
			$.ajax({
				url: furl + warName + "/getPbktDayList",
				type: "post",
				...
				$("#PbktCount").text(res[1].PbktCount);
		...
		$.ajax({
				url: furl + warName + "/getPbktRate",
				type: "post",
				data: {"school_id":school_id,"starttime":starttime,"endtime":endtime},
				datatype: "json",
				success: function (res) {
					var tmp1 ="{value:"+res[0].hd+",name:'互动行为'}";
					var tmp2 ="{value:"+res[1].tj+",name:'探究行为'}";
					var tmp3 ="{value:"+res[2].zd+",name:'作答行为'}";
					dataA.push(eval('(' + tmp1 + ')'));
					dataA.push(eval('(' + tmp2 + ')'));
					dataA.push(eval('(' + tmp3 + ')'));
