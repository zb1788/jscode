...
$(function() {
	// 初始化内容
	var url = furl + warName + "/getZYTeacherList?school_id="+school_id+"&starttime="+starttime+"&endtime="+endtime;
	setTable(url);
	setTime();
	setSelect("");
	echartBK();
...
function setTable(url){
  ...
  table.render({
    ...
    ,cols: [[
	  ...
	  ,{field:'hw_times', title: '发送次数', sort: true}
      ,{field:'person_times', title: '参与人次'}
      ,{field:'finish_times', title: '完成人次',sort: true}
      ,{field:'finish_rate', title: '完成率',sort: true,templet:function(d){
		  return d.finish_rate+'%';
	  }}
	  ,{field:'mark_rate', title: '批改率',sort: true,templet:function(d){
		  return d.mark_rate+'%';
	  }}
    ]]
	,done:function(res){
	...
	table.on('sort(test-table-thead1)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
	  ...
	  table.reload('tableList', {
		...
		,url: getSearchUrl("getZYTeacherList")
...
function search(){
	var url=getSearchUrl("getZYTeacherList");
	setTable(url);
}
   //导出
function excl_rep(){

	var url=getSearchUrl("getZYTeacherListExport");
				...
				if(data.data.length > 0){
					for(var i=0;i<data.data.length;i++){
						dataA.push([
						  ...
						  data.data[i].mark_rate.toString()+"%"
						]);
					}
				}
				excel.exportExcel({
...
function getSearchUrl(method){
	...
	var zyfs = $("#zyfsList option:selected").val();
	var url=furl + warName+"/"+ method +"?school_id="+school_id+"&starttime=" + starttime + "&endtime=" + endtime + "&gtId=" + gtId + "&search=" + search + "&orderBy=" + orderBy + "&zyfs=" + zyfs;
	return url;
}
/** 
 * 获取指定的URL参数值 
...
function getParam(paramName) { 
	...
	function echartBK() {
		...
		var option = {
			...
			$.ajax({
				url: furl + warName + "/getZYTeacherDayList",
				type: "post",
				...
				success: function (res) {
					for(var i in res[0].list){
						...
						dataY.push(res[0].list[i].teacherNum);
						dataZ.push(res[0].list[i].hw_times);
					}
				$("#HdxlCount").text(res[1].ZYTeacherCount);
				$("#hwCount").text(res[1].zyCount);
		...
		$.ajax({
				url: furl + warName + "/getZYTeacherRate",
				type: "post",
				data: {"school_id":school_id,"starttime":starttime,"endtime":endtime},
				datatype: "json",
				success: function (res) {
					var tmp1 ="{value:"+res[0].ZYTeacherCount+",name:'使用作业功能的教师'}";
					var tmp2 ="{value:"+res[1].unZYTeacherCount+",name:'未使用作业功能的教师'}";
