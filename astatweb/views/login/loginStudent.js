...
$(function() {// 初始化内容
	var url = furl + warName + "/getStudentLoginList?school_id="+school_id+"&starttime="+starttime+"&endtime="+endtime;
	setTable(url);
	setTime();
	echartBK();
});
function setTable(url){
  ...
  table.render({
    ...
    ,cols: [[
		...
		,{field:'truename', title: '用户'}
      ,{field:'class_name', title: '班级'}
	  ,{field:'teacher_name', title: '班主任'}
      ,{field:'login_times', title: '登录总次数', sort: true}
      ,{field:'online_time', title: '在线总时长（分钟）', sort: true}
	  ,{field:'por_login_times', title: '空间登录次数', sort: true}
	  ,{field:'app_login_times', title: '手机APP登录次数', sort: true}
    ]]
	,done:function(res){
        tdTitle();
    }
	...
	table.on('sort(test-table-thead1)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
	  ...
	  table.reload('tableList', {
		...
		,url: getSearchUrl("getStudentLoginList")
...
function search(){
	var url=getSearchUrl("getStudentLoginList");
	setTable(url);
}
   //导出
function excl_rep(){

	var url=getSearchUrl("getStudentLoginListExport");
	...
	$.ajax({
		...
		success: function(data){
			...
			layui.use(['excel'], function() {
				...
				var dataA = [
				  ['用户','班级','班主任','登录总次数','在线总时长（分钟）','空间登录次数','手机APP登录次数']
				];
				if(data.data.length > 0){
					for(var i=0;i<data.data.length;i++){
						dataA.push([
						  ...
						  data.data[i].online_time,
						  data.data[i].por_login_times.toString(),
						  data.data[i].app_login_times.toString()
						]);
					}
				}
				excel.exportExcel({
...
function getSearchUrl(method){
	...
	var dlqk = $("#dlqkList option:selected").val();
	var url=furl + warName+"/"+ method +"?school_id="+school_id+"&starttime=" + starttime + "&endtime=" + endtime + "&search=" + search + "&orderBy=" + orderBy + '&dlqk=' + dlqk;
	return url;
}
/** 
 * 获取指定的URL参数值 
...
function getParam(paramName) { 
	...
	function echartBK() {
		...
		var option = {
			...
			$.ajax({
				url: furl + warName + "/getStudentLoginDayList",
				type: "post",
				...
				success: function (res) {
					for(var i in res[0].list){
						...
						dataY.push(res[0].list[i].login_times);
					}
				$("#StudentLoginCount").text(res[1].StudentLoginCount);
		...
		$.ajax({
				url: furl + warName + "/getStudentLoginRate",
				type: "post",
				data: {"school_id":school_id,"starttime":starttime,"endtime":endtime},
				datatype: "json",
				success: function (res) {
					var tmp1 ="{value:"+res[0].StudentLoginCount+",name:'登录学生数'}";
					var tmp2 ="{value:"+res[1].unStudentLoginCount+",name:'未登录学生数'}";
