...
import { delRulesByIdList } from '@/api/rule';
...
export default {
  ...
  data () {
    return {
      ...
      operationViewList: [
        { id: '0', path: '/rule', label: '规则分组管理', __closeAble: false }
      ],
                ...
                <vcom-button
                  ...
                  onClick={async () => {
                    ...
                    this.checkedRows.map((item) => {
                      ruleList.push(item.ruleCode);
                    });
                    const res = await delRulesByIdList(ruleList.join(','));
                    if (res.code === '200') {
