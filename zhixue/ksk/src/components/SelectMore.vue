<template>
  <div class="timu" style="display: block;">
    <div class="padR">
      ...
      <template v-if="question.info.quesChoiceType===1">
        <ul class="listIww">
            <li 
              ...
              :key="item.flag"
              @click="choose(index)"
              :class="[chooseArr.indexOf(index)>-1?'cur':'']"
      ...
      <template v-if="question.info.quesChoiceType===2">
        <ul class="listImage tp">
          <li 
            :class="['table',chooseArr.indexOf(index)>-1?'cur':'']"
            v-for="(item,index) in question.info.quesChoice" 
            :key="item.flag"
            @click="choose(index)"
          >
    ...
    <div class="padR" v-if="isShowAnswer">
      <h6 class="textH6" >
        您选择的答案<font class="redFont">{{answerFormat}}</font> ,
    ...
    <div class="padR">
      ...
      <p><label>解析:</label></p>
      <p v-html="question.info.jiexi"></p>
      </div>
    </div>
...
export default {
  ...
  computed: {
    isReadOnly() {
      if (this.isComplete) {
        //查看上一题
        this.chooseArr = this.$store.state.userSelectAnswerArr[this.nowIndex];
    ...
    answerFormat() {
      var arr = this.$store.state.userSelectAnswerArr[this.nowIndex];
      var str = "";
      for (var i in arr) {
        str += this.answerArr[arr[i]];
      }
      return str;
    }
    // isDo() {
  ...
  methods: {
    ...
    choose: function(index) {
      ...
      if (!this.isComplete) {
        ...
        // if (!this.isDo) {
        //第一次选择
        if (this.chooseArr.indexOf(index) > -1) {
          this.chooseArr.splice(this.chooseArr.indexOf(index), 1);
        } else {
    ...
    submitQues() {
      ...
      this.isDo = true;
      this.add(this.chooseArr);
      this.$store.state.writeNum = this.nowIndex;
      var userAnswerStr = this.chooseArr.sort().join("");
      var answerStr = "";
      var rightAnswerArr = this.question.info.answer.sort();
      for (var i in rightAnswerArr) {
        if (rightAnswerArr[i] == "A") {
          answerStr += "0";
        } else if (rightAnswerArr[i] == "B") {
          answerStr += "1";
        } else if (rightAnswerArr[i] == "C") {
          answerStr += "2";
        } else if (rightAnswerArr[i] == "D") {
          answerStr += "3";
        } else if (rightAnswerArr[i] == "E") {
          answerStr += "4";
        } else if (rightAnswerArr[i] == "F") {
          answerStr += "5";
        }
      }
      ...
      this.$emit("addAnswerLog", this.question.id, this.isRight, userAnswerStr);
